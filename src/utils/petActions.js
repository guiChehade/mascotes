...
export const registerEntrada = async (petId, currentUser, service) => {
  ...
  try {
    ...
    // Obter os dados atuais do pet
    const petRef = doc(firestore, 'pets', petId);
    const petDoc = await getDoc(petRef);
    const petData = petDoc.data();
    const oldLocation = petData.localAtual || null;
    const petName = petData.mascotinho || '';
    ...
    // Atualizar o estado local do pet no banco de dados
    await updateDoc(petRef, { localAtual: service });

    // Atualizar a coleção 'locations'
    await updatePetLocation(petId, petName, oldLocation, service);
...
export const registerVoltaServico = async (petId, currentUser, service) => {
  ...
  try {
    ...
    // Atualizar o estado local do pet no banco de dados
    await updateDoc(petRef, { localAtual: initialService });

    // Atualizar a coleção 'locations'
    await updatePetLocation(petId, petName, oldLocation, initialService);
...
export const registerSaida = async (petId, currentUser) => {
  ...
  try {
    ...
    // Atualizar o estado local do pet no banco de dados
    await updateDoc(petRef, { localAtual: "Casa" });

    // Atualizar a coleção 'locations'
    await updatePetLocation(petId, petName, oldLocation, "Casa");
...
export const registerComentario = async (
  ...
  if (selectedComentarioType === 'Alimentação') {
    dataToSave = {
      ...comentarioData, // Inclui mealTime, feedingStatus, observations
    ...
    dataToSave = {
      comentario: comentarioData, // Texto do comentário
      usuario: currentUser.name,
      horario: formattedTime,
      data: formattedDate,
    };
  ...
  try {
    await addDoc(comentariosRef, dataToSave);
    return {
      success: true,
      message: `${selectedComentarioType} registrado com sucesso.`,
    };
  } catch (error) {
    console.error(
...
// Função para atualizar a localização do pet na coleção 'locations'
export const updatePetLocation = async (petId, petName, oldLocation, newLocation) => {
  try {
    // Remover do local antigo
    if (oldLocation) {
      const oldLocationRef = doc(firestore, 'locations', oldLocation);
      await updateDoc(oldLocationRef, {
        petIds: arrayRemove(petId),
        petNames: arrayRemove(petName),
      });
    }

    // Adicionar ao novo local
    if (newLocation) {
      const newLocationRef = doc(firestore, 'locations', newLocation);
      await setDoc(newLocationRef, {}, { merge: true }); // Garante que o documento exista
      await updateDoc(newLocationRef, {
        petIds: arrayUnion(petId),
        petNames: arrayUnion(petName),
      });
    }
  } catch (error) {
    console.error('Erro ao atualizar localização do pet:', error);
  }
};
